		...
		async (req: any, res: any) => {
			...
			try{
				...
				*/
				...
				let result = "";
				let messageVerb = "";
				if(req.body.action === 'addNewDevice' && PLMConfigNode.plm){
					let result = await PLMConfigNode.plm.linkDevice(address);
					let messageVerb = "linked";
					/* Get device info after we've added it */
					// let deviceInfo = await PLMConfigNode.plm!.queryDeviceInfo(address);
				}else if(req.body.action === 'removeDevice' && PLMConfigNode.plm){
					/* Get device info before we remove it */
					// let deviceInfo = await PLMConfigNode.plm!.queryDeviceInfo(address);
					let result = await PLMConfigNode.plm!.unlinkDevice(address);
